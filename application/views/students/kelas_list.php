  ...
  <?php foreach ($record as $kelas) {
    echo "<div class='class-box mb-4 pb-2'>";
      ...
      foreach ($kelas['sub_kelas'] as $sub_kelas):
        echo "<a href='".base_url()."students/learning/index/$sub_kelas[sub_kelas_id]' class='btn btn-sm btn-xs btn-block mt-2 shadow-sm' id=''><i class='fas fa-chalkboard-teacher mr-2'></i> <span class='text-hide-collapsed'>$sub_kelas[nama_sub_kelas]</span></a>";
      endforeach;
...
<div class="clearfix">
  <p class="text-center">
    <button class="btn btn-primary btn-sm mt-2 mb-3" id="join_class"><i class="fas fa-plus-circle"></i><span class='text-hide-collapsed'>Join Class</span></button>
  </p>
</div>